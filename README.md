...
* Configurable ESP32 and ESP32-S3 display profiles
...
The default configuration exposes 36 application objects:
...
| `BO1–BO4`  | General Binary Outputs                                         |

These are logical default roles. Actual BACnet instance numbers, names, descriptions, units, initial values, COV increments, and binary text are configurable through the parallel arrays in `User_Settings.c`.
...
Flash and monitor using the ESP-IDF extension or pass the corresponding arguments to the wrapper.

Select `esp32` or `esp32s3` as appropriate and choose the required display profile in:

```text
Project hardware → Display profile
```

See [`SETUP.md`](SETUP.md) for the complete environment and build procedure.

## Persistence

BACnet object properties and supported sensor configuration values are stored in NVS.
...
To erase saved values and restore the compiled defaults on the next boot:

```c
USER_OVERRIDE_NVS_ON_FLASH = 1;
```

After restoring the defaults, return the setting to `0`; otherwise NVS will be erased on every startup.
...
├── bacnet/              BACnet runtime, coordinator and event bus
...
![SEN54 sensor and display](docs/images/SEN54.jpg)
### BACnet objects in YABE

![BACnet objects in YABE](docs/images/YABE%20SEN54.png)

![](docs/images/20260724_080530.jpg)
![](docs/images/20260722_124532.jpg)
